...
def prompt_config(checkpoint_dir: Path, tokenizer: Tokenizer) -> Tuple[str, Tuple[List[int], ...]]:
    ...
    if re.search(r"vicuna|longchat", checkpoint_name):
        ...
        return system_prompt, stop_tokens
    if re.search("Llama-2", checkpoint_name):
        b_inst, e_inst = "[INST]", "[/INST]"
        b_sys, e_sys = "<<SYS>>\n", "\n<</SYS>>\n\n"
        system_prompt = (
            f"{b_sys}You are a helpful, respectful and honest assistant. Always answer as helpfully as possible, while"
            " being safe.  Your answers should not include any harmful, unethical, racist, sexist, toxic, dangerous,"
            " or illegal content. Please ensure that your responses are socially unbiased and positive in"
            " nature.\n\nIf a question does not make any sense, or is not factually coherent, explain why instead of"
            " answering something not correct. If you don't know the answer to a question, please don't share false"
            f" information.{e_sys}{b_inst} {{prompt}} {e_inst} "
        )
        stop_tokens = ([tokenizer.eos_id],)
        # TODO: check proper prompting for llama 2
        return system_prompt, stop_tokens
