...
class TrainArgs:
    ...
    def __post_init__(self) -> None:
        if self.lr_warmup_fraction and self.lr_warmup_steps:
            print(
                "Can't provide both `--train.lr_warmup_fraction` and `--train.lr_warmup_steps`. Defaulting to percentage."
            )
            self.lr_warmup_steps = None

        if not self.lr_warmup_fraction and not self.lr_warmup_steps:
            raise ValueError(
                "Must provide either `--train.lr_warmup_fraction` or `--train.lr_warmup_steps`. Choose one."
            )
    ...
    def warmup_iters(self, devices: int, max_iters: int, train_dataloader) -> int:
        ...
        if self.lr_warmup_fraction:
            return min(max_iters, math.ceil(self.lr_warmup_fraction * max_iters))
