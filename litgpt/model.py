    ...
    def max_seq_length(self, value: int) -> None:
        ...
        for l_ix, block in enumerate(self.transformer.h):
            ...
            if kv_cache is not None and isinstance(kv_cache, DenseKVCache) and kv_cache.cache_length < value:
                print(f"KV cache for layer {l_ix} too small: Call 'set_kv_cache(batch_size={kv_cache.batch_size}, max_seq_length={value}) before inference")
                break
        # Mask cache
...
class CausalSelfAttention(nn.Module):
    ...
    def forward(
        ...
        return_scores = not (input_pos is None or for_prefill) and self.kv_cache.update_requires_attn_weights()
