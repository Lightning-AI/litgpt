...
class GPT(nn.Module):
    ...
    def forward(
        ...
        - Training: `input_pos=None`. KV cache is not needed.
        - Inference, `input_pos` is given. There are two cases: `input_pos=0`
          (prefill) and `input_pos > 0` (generstion). For prefill, if
          KV caches are not present, they are created here (using default dense
          KV caches which store all K, V information). We must have
          `T <= model.kv_cache_max_prefill_length()`.
        - For generation, KV caches must be given, they are used and updated.
          We check that`input_pos == kv_cache.next_token_pos`. Note that
          `T > 1` is permitted here as well.
            ...
            if not for_prefill:
                ...
                for l_ix, block in enumerate(self.transformer.h):
                    ...
                    if kv_cache is None or kv_cache.next_token_pos is None:
                        raise ValueError(f"KV cache for layer {l_ix} is missing or not initialized. You need to start with pre-filling (input_pos=0)")
        ...
        for l_ix, block in enumerate(self.transformer.h):
            if for_prefill:
                ...
                elif attn.kv_cache.batch_size < eff_batch_size:
                    if self._default_kv_cache:
                        print(f"Re-allocating KV cache for layer {l_ix} (batch size was too small)")
                        create_def_cache = True
                    else:
                        raise ValueError(f"Batch size {eff_batch_size} is too large for KV cache layer {l_ix} (batch size {attn.kv_cache.batch_size})")
                if create_def_cache:
        ...
        if input_pos is not None:
            ...
            if not for_prefill:
                if self.kv_cache is None or self.kv_cache.next_token_pos is None:
                    raise ValueError("If input_pos is given, KV cache must exist and be initialized (call with input_pos=0)")
                ...
                raise ValueError("Internal error: KV cache must exist")
