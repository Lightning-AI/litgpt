...
def main(
    ...
    if resume:
        ...
        try:
            resume = max(
                out_dir.rglob("step-*/*.pth.adapter_v2"),
...
def fit(
    ...
    while step_count < max_steps and train_iterator.epoch < train.epochs:
        ...
        is_accumulating = (
            iter_num % train.gradient_accumulation_iters(devices, num_nodes) != 0
        )
        ...
        if not is_accumulating and step_count % eval.interval == 0:
            ...
            val_time_tensor = torch.tensor(
                t1, device=fabric.device, dtype=torch.float32
            )
            ...
            metrics = {
                ...
                "val_ppl": math.exp(val_loss_tensor),
            }
            fabric.log_dict(metrics, step=iter_num)
            fabric.barrier()
        ...
        if (
            ...
            checkpoint_file = (
                out_dir / f"step-{step_count:06d}" / "lit_model.pth.adapter_v2"
            )
