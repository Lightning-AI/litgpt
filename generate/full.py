...
def main(
    ...
    input: str = "",
    finetuned_path: Path = Path("out/full/alpaca/lit_model_full_finetuned.pth"),
    ...
    This script will only work with checkpoints from the instruction-tuned Parrot model.
    See `finetune/full.py`.

    Args:
        prompt: The prompt/instruction (Alpaca style).
        finetuned_path: Path to the checkpoint with finetuned model weights, which are the output of
            `finetune/full.py`.
    ...
    fabric.print(f"Loading model {str(finetuned_path)!r} with {config.__dict__}", file=sys.stderr)
    t0 = time.time()
    ...
    t0 = time.time()
    with lazy_load(finetuned_path) as finetuned_checkpoint:
        model.load_state_dict(finetuned_checkpoint, strict=False)
