...
def test_compute_warmup_iters():
    ...
    assert (
        train.warmup_iters(
            devices=1, num_nodes=1, max_iters=1000, train_dataloader=range(10)
        )
        == 0
    )
    ...
    with pytest.raises(
        ValueError, match="Can't provide both `--train.lr_warmup_fraction`"
    ):
    ...
    assert (
        train.warmup_iters(
            devices=2, num_nodes=1, max_iters=1000, train_dataloader=range(10)
        )
        == 200
    )
    ...
    assert (
        train.warmup_iters(
            devices=1, num_nodes=1, max_iters=120, train_dataloader=range(10)
        )
        == 120
    )
    ...
    assert (
        train.warmup_iters(
            devices=1, num_nodes=1, max_iters=1000, train_dataloader=range(100)
        )
        == 30
    )
    ...
    assert (
        train.warmup_iters(
            devices=1, num_nodes=1, max_iters=20, train_dataloader=range(100)
        )
        == 20
    )
    ...
    assert (
        train.warmup_iters(
            devices=1, num_nodes=1, max_iters=1000, train_dataloader=range(5)
        )
        == 2
    )
